...
from qlgates.helpers import build_local_operators,expectation, loschmidt_amplitude
...
#    title=fr"$\delta t = {cfg.deltat},\; T = {cfg.timesteps * cfg.deltat}\,\mathrm{{fs}}$")
        ...
        l_ampl,rate = loschmidt_amplitude(cfg,psit[:,0], psit)
        print(f"Loschmidt amplitude and rate calculated for h={h}.")
        simpleplot(h*t,rate,
        filename=f"./T{cfg.timesteps * cfg.deltat}_dt{cfg.deltat}_h{h}_J{cfg.J}_ql_N{cfg.NQL}_loschmidtrate.png",
        xlabel=r"$\tau$",
        ylabel=r"$\lambda (t)$",
        title=fr"$\delta t = {cfg.deltat},\; T = {cfg.timesteps * cfg.deltat}\,\mathrm{{au}}$; h={h}, J={cfg.J}, N={cfg.NQL}",
        marker=None)

    Meq = np.array(M_eq)
    simpleplot(ratios,Meq,
    filename=f"./T{cfg.timesteps * cfg.deltat}_dt{cfg.deltat}_J{cfg.J}_cl_N{cfg.NQL}_magnetization_test.png",
    xlabel="h/J",
    ylabel="Magnetization (time-averaged)",
    title=fr"$\delta t = {cfg.deltat},\; T = {cfg.timesteps * cfg.deltat}\,\mathrm{{fs}}$")
